  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <KpiCard label="Total Users" value={stats.totalUsers} color="blue" />
        <KpiCard label="Total Projects" value={stats.totalProjects} color="green" />
        <KpiCard label="Total Syncs" value={stats.totalSyncs} color="purple" />
        <KpiCard label="Sync Success Rate" value={`${stats.syncSuccessRate}%`} color={stats.syncSuccessRate >= 95 ? "green" : stats.syncSuccessRate >= 80 ? "yellow" : "red"} />
      </div>
        ...
        <div className="flex gap-4">
          {stats.syncBreakdown.map((item) => (
            <div key={item.status} className="bg-gray-800 rounded-lg px-4 py-3 min-w-[120px]">
              ...
              <div className="text-2xl font-bold text-white">{item._count.status}</div>
            </div>
          ))}
        </div>
      </div>
